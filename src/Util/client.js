...
module.exports = class Naroya extends Client {
    ...
    constructor(clientOptions = {}) {
        ...
        clientOptions.partials = ['CHANNEL', 'GUILD_MEMBER', 'MESSAGE', 'REACTION', 'USER'];
        clientOptions.allowedMentions = {"repliedUser": false, "roles": false, "users": false};
    ...
    initialize = async() => {
        ...
        commandFiles.forEach(fileStr => {
            ...
            if(!file?.name) return console.log(`File ${fileStr.split("/")[-1]} doesn't have a name`);
            if(!file?.description) return console.log(`Command ${file.name} doesn't have a description`);
